...
export const listAdmin = query({
  ...
  handler: async (ctx) => {
    ...
    const rows = await ctx.db.query("unitCategories").collect();
...
export const remove = mutation({
  args: { id: v.id("unitCategories") },
  handler: async (ctx, { id }) => {
    await requireAdminOrCreator(ctx);
    for (const u of await ctx.db
      .query("units")
      .withIndex("by_unit_category", (q) => q.eq("unitCategoryId", id))
      .collect()) {
      if (isLive(u)) {
        throw new Error("Cannot delete: one or more units use this category");
      }
    }
